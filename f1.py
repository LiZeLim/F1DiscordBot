...
class F1:
    ...
    def create_teams(self) -> list[Team]:
        f1_teams_soup = self.url_to_soup_lxml(TEAMS_URL)
        ...
        for f in f1_teams_soup_list:
            ...
            team_name = self.extract_team_details(soup_str, NAME_PATTERN, NAME_PATTERN_STRIP_STR)
            team_color = self.extract_team_details(soup_str, COLOR_PATTERN, COLOR_PATTERN_STRIP_STR)
    ...
    def extract_team_details(self, soup_str : str, regex_extraction_pattern : str, strip_pattern : str) -> str:
    ...
    def curr_season_results(self) -> str:
        ...
        race_results_soup = self.url_to_soup_lxml(SEASON_URL.format(datetime.now().year))
    ...
    def prev_season_result(self, year: int,) -> tuple:  # tuple of str first half of season, str second half of season
    ...
    def driver_standings(self) -> str:
        drivers_df = pd.DataFrame([driver.__dict__ for driver in self.drivers])
    ...
    def constructors_standings(self) -> str:
        constructors_df = pd.DataFrame([team.__dict__ for team in self.teams])
